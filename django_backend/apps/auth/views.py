...
class close_delta_position(APIView):
    ...
    def post(self,request):
        ...
        client = DeltaExchangeClient(api_key=apikey,api_secret=apisecret)
        side = data['side']
        order = client.place_order(product_symbol = symbol,side = side,size=quantity)
        ...
        if position.broker.broker == "Coindcx":
            ...
            if (position.side == "buy" and quantity_balance > 0) or (position.side == "sell" and quantity_balance < 0):
                ...
                if order['success'] == True:
                    ...
                    return Response({'error': 'Failed to place order'}, status=400)
...
# Broker order execution functions removed - should be in Strategy Engine, not Django

# ProcessSignal removed - broker order execution logic should be in Strategy Engine, not Django
...
class setSignal(APIView):
    ...
    def post(self,request):
        ...
        if data['typeq'] == "market":
            ...
            trailing_pts = float(data.get('trailingpoints', 0) or 0)
            ...
            # Handle trailingpoints - default to 0 if missing
            trailing_pts_value = data.get('trailingpoints', 0) or 0
            copy = copysignal(owner = request.user, is_trailing = is_trailing, symbol = data['symbol'],side = data['side'],target = data['target'],stoploss = data['stoploss'],entry = float(price1),typeq = data['typeq'],strategy_id = data['strategy_id'],url = internal_signal_url,trailingpoints = trailing_pts_value,status = "Active",trailingprice =tpprice,symbolid = data['symbolid'] ,leverage = data['leverage'],capital = data['capital'])

            copy.save()
            
            # Build pipe-separated payload for Strategy Engine
            # Format: symbol|symbolid|entry|target|stoploss|strategy_id|strategy_code|tradingbridgecode|side|datetime|leverage|capital|type|blank
            signal_parts = [
                data['symbol'],
                str(data['symbolid']),
                str(price1),  # entry price (live price for market orders)
                str(data['target']),
                str(data['stoploss']),
                str(data['strategy_id']),
                str(data['strategy_code']),
                "DELTA",
                data['side'],
                formatted_datetime,
                str(data['leverage']),
                str(data['capital']),
                "Entry",
                ""  # blank field (14th field)
            ]
            signal_payload = "|".join(signal_parts) + "|"
            
            logger.info(f"[setSignal] ✅ DB saved: copysignal ID={copy.id}")
            logger.info(f"[setSignal] Pipe string built: {signal_payload[:100]}...")
            
            return Response({'message':'Signal saved successfully.'},status=status.HTTP_200_OK)
            ...
            # Handle trailingpoints - default to 0 if missing
            trailing_pts_value = data.get('trailingpoints', 0) or 0
            copy = copysignal(owner = request.user, is_trailing = is_trailing, symbol = data['symbol'],side = data['side'],target = data['target'],stoploss = data['stoploss'],entry = float(data['entry']),typeq = data['typeq'],strategy_id = data['strategy_id'],url = internal_signal_url,trailingpoints = trailing_pts_value,status = "Pending",trailingprice =tpprice,symbolid = data['symbolid'] ,leverage = data['leverage'],capital = data['capital'])
            ...
            signal_parts = [
                data['symbol'],
                str(data['symbolid']),
                str(data['entry']),  # entry price (from request for limit orders)
                str(data['target']),
                str(data['stoploss']),
                str(data['strategy_id']),
                str(data['strategy_code']),
...
class add_strategy(APIView):
    ...
    Creates a strategy in the database.
    Accepts strategy data and saves to highLowstratergy model.
    """
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        try:
            ...
            # SAFE + CORRECT LOGIC: Use JWT user as primary source (security best practice)
            # Payload fields are optional fallbacks only
            owner = data.get('owner') or getattr(user, 'phone', None) or str(user_id)
            phone = data.get('phone') or owner
            email = data.get('email') or getattr(user, 'email', None) or ''
            
            # Protection: Validate phone is available
            if not phone:
                return Response({
                    'success': False,
            ...
            indicator_name = data.get('indicator_name', strategy_type)
            
            target = data.get('target', None)
            sl = data.get('sl', None)
            
            logic = data.get('logic')  # Can be JSON string or object
            risk = data.get('risk')  # Can be JSON string or object
            tags = data.get('tag', [])
            
            entry_condition = data.get('entry_condition', '')
            exit_condition = data.get('exit_condition', '')
            
            # STEP 3: PROTECTION - Validate required fields
            ...
            # Generate strategy code if not provided (fast operation)
            strategy_code = data.get('stratergy_code') or data.get('strategy_code') or f'STRG-{random.randint(100000, 999999)}'
            
            # Normalize symbol (fast operation)
            symbol = symbol.upper()[:25] if symbol else 'BTCUSD'
            
            # MINIMAL DB SAVE - Fast response (<1 second)
            # Create strategy in database (single operation, no extra saves, no M2M operations)
            strategy = highLowstratergy.objects.create(
                owner=str(user_id),  # Use authenticated user from JWT
                stratergy_code=strategy_code,
                name=strategy_name[:25] if strategy_name else 'Untitled Strategy',
                full_name=data.get('full_name', strategy_name)[:25] if data.get('full_name') or strategy_name else 'Untitled Strategy',
                symbol=symbol,
                stratergy_description=data.get('stratergy_description') or data.get('strategy_description') or f'AI Generated {strategy_type} strategy',
                risk='medium',
                strategy_allow=data.get('strategy_allow', 'limited'),
                is_active=False,  # Draft strategies are not active
                tag=tags,
                trading_type=data.get('trading_type', 'Automatic'),
                created_date=timezone.now().date(),
                overallReturn=0.000,
                captial_requirement=data.get('capital_requirement') or data.get('captial_requirement', 'NA'),
            ...
            # Removed operations to prevent timeout:
            # - send_mail() - REMOVED (can block 2-5 seconds)
            # - cache.delete_pattern() - REMOVED (can be slow)
            # - strategy.allowed_users.add() - REMOVED (M2M can be slow, can be done async)
            # - JSON parsing of logic/risk - REMOVED (not needed for save, can be slow)
            # - Extra strategy.save() - REMOVED (unnecessary)
        ...
        # Note: Django handles DB connections automatically via connection pooling
        # Removing connection.close() to prevent potential blocking
...
class StrategyCreateView(APIView):
    ...
    Creates a strategy in the database with combined data from highlow-strategies API and generated strategy.
    ...
    - timeframe: str
    - strategy_type: str
    - logic: str (JSON string)
    - risk: str (JSON string)
    
    Backend sets:
    - user_id: from JWT token (request.user.id)
    - status: "draft"
    ...
    def post(self, request):
        try:
            # IMPORTANT: Explicitly reject id, owner, is_active from frontend for security
            # These are set by the backend based on authenticated user and status="draft"
            if 'id' in request.data:
                return Response({
                    'success': False,
                    'message': 'Field "id" is not allowed. It is auto-generated by the backend.'
                }, status=status.HTTP_400_BAD_REQUEST)
            
            if 'owner' in request.data:
                return Response({
                    'success': False,
                    'message': 'Field "owner" is not allowed. It is set from authenticated user.'
                }, status=status.HTTP_400_BAD_REQUEST)
            
            if 'is_active' in request.data:
                return Response({
                    'success': False,
                    'message': 'Field "is_active" is not allowed. New strategies are created with status="draft" (is_active=False).'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            strategy_type = request.data.get('strategy_type')
            logic = request.data.get('logic')  # Can be JSON string or object
            risk = request.data.get('risk')  # Can be JSON string or object
            
            # Validate required fields
            if not all([strategy_name, symbol, timeframe, strategy_type, logic, risk]):
            ...
            # Get user_id from JWT token (DO NOT accept from request body)
            user_id = request.user.id
            
            # Parse JSON strings to validate they are valid JSON
            try:
                logic_json = json.loads(logic) if isinstance(logic, str) else logic
                risk_json = json.loads(risk) if isinstance(risk, str) else risk
            except json.JSONDecodeError as e:
                return Response({
                    'success': False,
                    'message': f'Invalid JSON in logic or risk field: {str(e)}'
                }, status=status.HTTP_400_BAD_REQUEST)
            
            # Create strategy entry in highLowstratergy model
            # Map fields from request to model fields
            # Accept both correct spelling (strategy_code) and typo (stratergy_code) for compatibility
            strategy_code = (
                request.data.get('strategy_code') or 
                request.data.get('stratergy_code') or 
                f'STRG-{random.randint(100000, 999999)}'
            )
            full_name = request.data.get('full_name') or strategy_name
            # Accept both correct spelling (strategy_description) and typo (stratergy_description)
            description = (
                request.data.get('strategy_description') or 
                request.data.get('stratergy_description') or 
                f'AI Generated {strategy_type} strategy'
            )
            
            # Create strategy with status="draft"
            strategy = highLowstratergy.objects.create(
                owner=str(user_id),  # Store user_id as owner
                stratergy_code=strategy_code,
                name=strategy_name[:25],  # Truncate to max_length
                full_name=full_name[:25],  # Truncate to max_length
                symbol=symbol.upper()[:25],
                stratergy_description=description[:500],  # Truncate to max_length
                risk='medium',  # Default risk level (risk JSON stored separately)
                strategy_allow=request.data.get('strategy_allow', 'limited'),
                is_active=False,  # Draft strategies are not active
                tag=request.data.get('tag', []),
                trading_type=request.data.get('trading_type', 'Automatic'),
                created_date=timezone.now().date(),
                overallReturn=0.000,
                # Accept both correct spelling (capital_requirement) and typo (captial_requirement)
                captial_requirement=request.data.get('capital_requirement') or request.data.get('captial_requirement', 'NA'),
                entry_time=request.data.get('entry_time', 'NA'),
                exit_time=request.data.get('exit_time', 'NA'),
                target=request.data.get('target', 'NA'),
                sl=request.data.get('sl', 'NA'),
            )
            
            # Store additional AI strategy data in stratergy_description as JSON
            # Format: {original_description, strategy_type, logic, risk_json}
            ai_strategy_data = {
                'original_description': description,
                'strategy_type': strategy_type,
                'timeframe': timeframe,
                'logic': logic_json if isinstance(logic_json, dict) else json.loads(logic) if isinstance(logic, str) else logic,
                'risk_json': risk_json if isinstance(risk_json, dict) else json.loads(risk) if isinstance(risk, str) else risk,
                'status': 'draft',
                'created_at': timezone.now().isoformat(),
                'user_id': user_id
            }
            
            # Update description to include AI strategy data (store as JSON string in description field)
            strategy.stratergy_description = json.dumps(ai_strategy_data)[:500]  # Truncate if too long
            
            # Add current user to allowed_users
            ...
            logger.info(f'Strategy created successfully: ID={strategy.id}, Code={strategy_code}, User={user_id}')
...
FASTAPI_PAYMENT_INTERNAL_URL = os.getenv(
    "FASTAPI_PAYMENT_INTERNAL_URL",
    "https://aistrategy.cryptoarth.in/payment/verify-internal"
)
