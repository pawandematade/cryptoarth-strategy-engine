...
from .views import *
from .views_live_performance import LivePerformanceSummaryView, LivePerformanceDailyView, LivePerformanceTradesView
...
urlpatterns = [
    ...
    # ProcessSignal route removed - broker order execution should be in Strategy Engine, not Django
...
path("get_user_positions/", get_user_positions.as_view(), name="get_user_positions"),
    ...
    path('tutorials/<int:pk>/', TutorialDetailAPIView.as_view(), name='tutorial-detail'),
    ...
    path('get_user_pnl/', get_user_pnl.as_view(), name='get_user_pnl'),
    path('broker/balance/', balanceFetch.as_view(), name='balanceFetch'),
    ...
    # Strategy create endpoint - saves AI-generated strategy to DB with status="draft"
    path('strategy/create/', StrategyCreateView.as_view(), name='strategy-create'),
    # Live Performance APIs (Step-10 - Read-only)
    path('strategy/<int:id>/live/performance/summary', LivePerformanceSummaryView.as_view(), name='live-performance-summary'),
    path('strategy/<int:id>/live/performance/daily', LivePerformanceDailyView.as_view(), name='live-performance-daily'),
    path('strategy/<int:id>/live/performance/trades', LivePerformanceTradesView.as_view(), name='live-performance-trades'),
    # TEMP: Payment verify endpoint (Django → FastAPI bridge) - REMOVE AFTER BACKEND MERGE
