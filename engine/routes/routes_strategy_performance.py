...
class PerformanceResponse(BaseModel):
    ...
    monthly_performance: Optional[Dict[str, Any]] = None  # Year â†’ Month â†’ Day grouping
...
def _run_backtest(strategy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Group trades by date (Year â†’ Month â†’ Day)
...
def _apply_brokerage_to_performance(
    ...
    for trade in performance.get('trades', []):
        ...
        # Position size = capital Ã— leverage Ã— (capital_percent / 100)
                ...
                try:
                    ...
                    if db_strategy:
                        ...
                        if latest_version:
                            ...
                            if execution:
                                logger.info(f"âœ… Backtest execution logged: strategy_id={strategy_id}, execution_id={execution.id}")
                            else:
                                logger.warning(f"âš ï¸ Backtest execution logging returned None for strategy_id={strategy_id}")
                        else:
                            logger.warning(f"âš ï¸ No version found for strategy_id={strategy_id}, cannot log backtest")
                    else:
                        logger.warning(f"âš ï¸ Strategy {strategy_id} not found in database, cannot log backtest")
                except Exception as log_error:
                    backtest_db.rollback()
                    logger.error(f"âŒ Failed to log backtest execution: {log_error}", exc_info=True)
                finally:
                    backtest_db.close()
            except Exception as log_error:
                # Don't fail backtest if logging fails, but log the error
                logger.error(f"âŒ Failed to create database session for backtest logging: {log_error}", exc_info=True)
