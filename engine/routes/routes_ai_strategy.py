...
def generate_ai_strategy(
    ...
    try:
        ...
        if (not prompt_value or not str(prompt_value).strip()) and description_value and str(description_value).strip():
            ...
            logger.info("âœ… Normalized 'description' to 'prompt' in endpoint")
            prompt_value = request.prompt
        
        # Validate required fields FIRST (before extra keys check)
        if not prompt_value or not str(prompt_value).strip():
            logger.error("âŒ Validation failed: Prompt is empty")
        ...
        logger.info("=" * 80)
        logger.info("ðŸ”„ NEW STRATEGY GENERATION REQUEST RECEIVED")
        ...
        if extra_keys:
            logger.error(f"âŒ REJECTED: Extra keys in payload: {extra_keys}")
        ...
        logger.info(f"ðŸ” CREDIT CHECK DEBUG: user.id={user.id}, user.external_user_id={user.external_user_id}")
        ...
        logger.info(f"ðŸ” CREDIT CHECK RESULT: is_available={is_available}, available_credits={available_credits}, required_credits={required_credits}")
        
        if not is_available:
            # Block generation if credits <= 0
            logger.warning(f"AI GENERATE BLOCKED â€“ insufficient credits: external_user_id={user.external_user_id}, available={available_credits}, required={required_credits}")
            raise HTTPException(
        ...
        logger.info(f"AI GENERATE CALLED â€“ credit deducted: user_id={user.id}, credits={required_credits}, remaining={available_credits - required_credits}")
        ...
        try:
            ...
            logger.error(f"âŒ PromptBuilder validation failed: {e}")
        ...
        try:
            logger.info("ðŸ¤– Calling OpenAI with merged prompt (all parameters embedded in single string)")
            ...
            if not strategy:
                logger.error("âŒ OpenAI returned None - strategy generation failed")
                raise HTTPException(
                    status_code=500,
            ...
            logger.info(f"âœ… Strategy generated and validated successfully")
            ...
            error_msg = str(e)
            logger.error(f"âŒ Strategy validation error: {error_msg}")
        ...
        if not strategy:
            logger.error("âŒ Strategy generation returned None. Check OpenAI service logs.")
        ...
        if strategy:
            ...
            if 'userParams' in strategy:
                ...
                logger.warning("âš ï¸ Removed userParams from strategy object (security violation)")
            ...
            if found_old_fields:
                logger.error(f"âŒ Old schema fields detected in OpenAI response: {found_old_fields}")
                raise HTTPException(
                    status_code=500,
            ...
            if missing_sections:
                logger.error(f"âŒ CRITICAL ERROR: Strategy missing required sections: {missing_sections}")
            ...
            if not logic:
                logger.error("âŒ CRITICAL ERROR: logic section is empty")
            ...
            if forbidden_fields:
                logger.warning(f"âš ï¸ Removing forbidden fields: {forbidden_fields}")
                ...
                for field in forbidden_fields:
                    if field not in required_sections:  # Double check - don't remove required sections
                        del strategy[field]
                        logger.warning(f"âš ï¸ Removed forbidden field '{field}' from strategy object")
            ...
            for field in forbidden_logic_fields:
                if field in logic:
                    del logic[field]
                    logger.warning(f"âš ï¸ Removed forbidden field '{field}' from logic section")
            ...
            logger.info(f"âœ… Using OpenAI response with unified schema (logic, risk, meta)")
            ...
            logger.info("=" * 80)
            logger.info("ðŸ“Š FINAL STRATEGY STRUCTURE (Unified Schema):")
        ...
        logger.info("=" * 80)
        logger.info("âœ… STRATEGY GENERATION COMPLETED SUCCESSFULLY (Runtime Only - No Storage)")
        logger.info(f"Strategy Type: {strategy.get('strategy_type') if strategy else 'None'}")
        ...
        if strategy:
            ...
            if forbidden_fields:
                logger.warning(f"âš ï¸ Strategy contains unexpected fields: {forbidden_fields}")
                # Remove only if not required
                for key in list(forbidden_fields):
                    if key not in required_sections:
                        del strategy[key]
                        logger.warning(f"âš ï¸ Removed forbidden field '{key}' from final strategy")
...
def run_strategy_backtest(
    ...
    try:
        ...
        logger.info(f"ðŸ” BACKTEST CREDIT CHECK DEBUG: user.id={user.id}, user.external_user_id={user.external_user_id}")
        is_available, available_credits, required_credits = check_credits_available(
            db, user.external_user_id, 'backtest'
        )
        
        logger.info(f"ðŸ” BACKTEST CREDIT CHECK RESULT: is_available={is_available}, available_credits={available_credits}, required_credits={required_credits}")
        ...
        logger.info(f"BACKTEST CALLED â€“ credit deducted: user_id={user.id}, strategy_code={strategy_code}, credits={required_credits}, remaining={available_credits - required_credits}")
...
def _group_trades_by_date(trades, candles_list):
    """Group trades by Year â†’ Month â†’ Day."""
...
def _run_preview_backtest(strategy: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Note: fetch_ohlcv now handles UI â†’ Delta mapping and chunked fetching automatically
