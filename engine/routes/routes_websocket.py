...
from common.config import DELTA_BASE_URL, IS_PRODUCTION, FRONTEND_URL
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket):
        ...
        logger.info(f"âœ… WS connected | total={len(self.active_connections)}")

    def disconnect(self, websocket: WebSocket):
        self.active_connections.discard(websocket)
        self.connection_subscriptions.pop(websocket, None)
        self.connection_loops.pop(websocket, None)
        logger.info(f"âŒ WS disconnected | total={len(self.active_connections)}")
...
def on_delta_message(ws, message):
    try:
        ...
        if data.get("type") == "v2/ticker":
            ...
            # âœ… FIX: Broadcast ticker even if ltp is 0 (use 'is not None' instead of truthy check)
            if ltp is not None:
                try:
                ...
                # âœ… FIX: Include ALL ticker fields (OHLC, Change %)
...
def subscribe_to_delta(ws, symbols: List[str], orderbook: bool):
    ...
    logger.info(f"ðŸš€ Delta subscribed symbols: {symbols}")
...
def connect_to_delta():
    ...
    def on_open(ws):
        logger.info("âœ… Delta WS connected")

        # ðŸ”¥ AUTO subscribe to already requested symbols
        if subscribed_symbols:
            logger.info(f"ðŸš€ Auto-subscribing on Delta connect: {list(subscribed_symbols)}")
    ...
    logger.info("âœ… Delta WS thread started")
            ...
            try:
                ...
                if message.get("type") == "subscribe":
                    ...
                    if "channels" in message:
                        logger.warning("âŒ Legacy subscribe ignored")
                        continue

                    # âœ… FIX: Handle product_ids from frontend subscribe message
                    ...
                    if not product_ids:
                        logger.warning(f"âŒ Subscribe message missing product_ids: {message}")
                        continue

                    logger.info(f"ðŸ“¥ Subscribe received: product_ids={product_ids}")
                    ...
                    if not delta_symbols:
                        logger.warning(f"âŒ No symbols found for product_ids: {product_ids}")
                        continue
                    ...
                    if delta_ws:
                        ...
                        logger.warning("âŒ Delta WebSocket not available, retrying connection...")
                        connect_to_delta()
