...
# âœ… SOURCE OF TRUTH: Candle schema from Python code
# Derived from:
...
def create_table_if_not_exists(symbol: str, engine_instance: Engine = engine) -> bool:
    ...
    try:
        ...
        logger.error(f"âŒ Failed to create table {table_name}: {e}", exc_info=True)
        return False
...
def insert_candles(
    ...
    try:
        ...
        logger.info(f"âœ… Inserted {inserted_count} candles into {table_name}")
        return inserted_count
        
    except Exception as e:
        logger.error(f"âŒ Failed to insert candles into {table_name}: {e}", exc_info=True)
        return 0
...
def get_candles(
    ...
    try:
        ...
        logger.error(f"âŒ Failed to retrieve candles from {table_name}: {e}", exc_info=True)
        return []
...
def get_last_candle(symbol: str, engine_instance: Engine = engine) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"âŒ Failed to retrieve last candle from {table_name}: {e}", exc_info=True)
        return None
...
def delete_candles(
    ...
    try:
        ...
        logger.info(f"âœ… Deleted {deleted_count} candles from {table_name}")
        return deleted_count
        
    except Exception as e:
        logger.error(f"âŒ Failed to delete candles from {table_name}: {e}", exc_info=True)
        return 0
