...
def store_backtest_summary(
    ...
    try:
        ...
        logger.error(f"âŒ Failed to store backtest summary: {e}", exc_info=True)
        raise
...
def store_backtest_daily(
    ...
    try:
        ...
        logger.error(f"âŒ Failed to store backtest daily data: {e}", exc_info=True)
        raise
...
def store_backtest_trades(
    ...
    try:
        ...
        logger.error(f"âŒ Failed to store backtest trades: {e}", exc_info=True)
        raise
...
def store_backtest_results(
    ...
    try:
        ...
        logger.info(
            f"âœ… Stored backtest results for strategy {strategy_id} {symbol} "
        ...
        db.rollback()
        logger.error(f"âŒ Failed to store backtest results: {e}", exc_info=True)
        raise
